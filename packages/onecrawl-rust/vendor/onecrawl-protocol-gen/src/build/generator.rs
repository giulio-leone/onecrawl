...
/// # use std::io::Result;
/// fn main() -> Result<()> {
///   onecrawl_protocol_gen::build::compile_pdls(&["src/js.pdl", "src/browser.pdl"])?;
///   Ok(())
/// }
/// ```
...
impl Generator {
    ...
    ///
    /// This method is like the `onecrawl_protocol_gen::build::compile_pdls`
    ...
    /// # use std::io::Result;
    /// fn main() -> Result<()> {
    ///   let mut pdl_build = onecrawl_protocol_gen::build::Generator::default();
    ...
    pub fn compile_pdls<P: AsRef<Path>>(&mut self, pdls: &[P]) -> io::Result<()> {
        ...
        let stream = quote! {
            ...
            /// `onecrawl_browser_types::MethodType` and it must be `DeserializeOwned`
            ...
            pub trait CustomEvent: ::std::any::Any + serde::de::DeserializeOwned + onecrawl_browser_types::MethodType + Send + Sync {
            ...
            pub(crate) mod sealed {
                ...
                pub trait SealedEvent: ArcAny + onecrawl_browser_types::MethodType {
    ...
    fn generate_type(&mut self, domain: &Domain, dt: DomainDatatype) -> TokenStream {
        let stream = if let Some(vars) = dt.as_enum() {
            ...
            if !dt.is_type() {
                stream.extend(quote! {
                    impl onecrawl_browser_types::Method for #name {

                        fn identifier(&self) -> onecrawl_browser_types::MethodId {
                            Self::IDENTIFIER.into()
                        }
                    }

                    impl onecrawl_browser_types::MethodType for #name {

                        fn method_id() -> onecrawl_browser_types::MethodId where Self: Sized {
                            Self::IDENTIFIER.into()
                        }
                    }
                });
            ...
            if let DomainDatatype::Command(cmd) = dt {
                ...
                stream.extend(quote! {
                    impl onecrawl_browser_types::Command for #name {
    ...
    fn generate_field_type(
        ...
        match ty {
            ...
            Type::Binary => (
                FieldType::new(quote! {onecrawl_browser_types::Binary}),
                Either::Left(size_of::<onecrawl_browser_types::Binary>()),
            ),
            Type::Enum(_) => {
