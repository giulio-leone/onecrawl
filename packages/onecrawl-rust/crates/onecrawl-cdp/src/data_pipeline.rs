    ...
    match step {
        PipelineStep::Filter {
            ...
            let filtered: Vec<_> = items
                .into_iter()
                .filter(|item| match_filter(item.get(field).map(|s| s.as_str()).unwrap_or(""), operator, value))
                .collect();
...
pub fn validate_pipeline(pipeline: &Pipeline) -> Vec<String> {
    ...
    for (i, step) in pipeline.steps.iter().enumerate() {
        match step {
            PipelineStep::Filter { field, operator, .. } => {
                if field.is_empty() {
                    errors.push(format!("step {i}: filter field is empty"));
                }
                let valid = [
                    "eq", "neq", "contains", "starts_with", "ends_with", "regex",
                    "gt", "lt", "empty", "not_empty",
                ];
            ...
            PipelineStep::Transform { field, transform } => {
                ...
                let valid = [
                    "trim", "lowercase", "uppercase", "strip_html", "strip_whitespace",
                    "truncate", "replace", "split",
                ];
...
fn match_filter(val: &str, operator: &str, target: &str) -> bool {
    match operator {
        ...
        "gt" => {
            match (val.parse::<f64>(), target.parse::<f64>()) {
                (Ok(a), Ok(b)) => a > b,
                _ => val > target,
            }
        }
